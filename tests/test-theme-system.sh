...
assert_contains "v1.2 audit records the accepted candidate commit" "dac5d4a62b8a978d5c3787c032f0aa1b7575d7f4" "$RELEASE_AUDIT_DOC"
assert_contains "v1.2 audit records the accepted candidate SHA" "319cc1472d8e7bd35d140b8b31038d05460eb8876c7079b5739dd4f4284ea174" "$RELEASE_AUDIT_DOC"
assert_contains "v1.2 audit records the supervised canary complete" "The real Codex/iTerm2 canary passed" "$RELEASE_AUDIT_DOC"
assert_contains "v1.2 audit leaves publication in issue #17" "issue #17" "$RELEASE_AUDIT_DOC"
assert_contains "v1.2 audit quarantines the uncleared candidate" "Publication status: BLOCKED — candidate quarantined" "$RELEASE_AUDIT_DOC"
assert_contains "v1.2 audit records all packaged branded PNGs" "55 packaged branded PNGs" "$RELEASE_AUDIT_DOC"
assert_contains "v1.2 audit distinguishes sprites from contact sheets" "53 theme sprites plus two contact sheets" "$RELEASE_AUDIT_DOC"
assert_contains "v1.2 audit records exact PokeAPI HOME provenance" "All 18 packaged Pokemon sprites byte-for-byte match PokeAPI HOME files" "$RELEASE_AUDIT_DOC"
assert_contains "v1.2 audit preserves assets during evidence review" "Preserve every current asset during the evidence slice" "$RELEASE_AUDIT_DOC"
assert_not_contains "v1.2 audit no longer recommends blanket asset removal" "branded-asset-free replacement candidate" "$RELEASE_AUDIT_DOC"
assert_contains "v1.2 audit forbids publishing the quarantined SHA" "must not be published" "$RELEASE_AUDIT_DOC"
assert_contains "v1.2 audit records npm auth as secondary" "npm authentication is a secondary blocker" "$RELEASE_AUDIT_DOC"
...
assert_not_contains "Closed supervised canary issue #16 leaves the open roadmap" "issues/16" "$SUPERVISED_ROADMAP"
...
assert_contains "Roadmap records the accepted correction commit" "dac5d4a62b8a978d5c3787c032f0aa1b7575d7f4" "$ROADMAP_DOC"
assert_contains "Roadmap records the accepted replacement candidate SHA" "319cc1472d8e7bd35d140b8b31038d05460eb8876c7079b5739dd4f4284ea174" "$ROADMAP_DOC"
assert_contains "Roadmap records the accepted correction CI" "32637312548" "$ROADMAP_DOC"
assert_contains "Roadmap records the completed exact-artifact Sol High canary" "82B1157B-A11E-4AF8-B029-D379249122F2" "$ROADMAP_DOC"
assert_contains "Roadmap records stable 12/12 DONE acceptance" "12/12 DONE" "$ROADMAP_DOC"
assert_contains "Roadmap records the accepted exact-artifact Nurse Joy composite" "e72f565423a40cc5d35393ebaac0886cf04a363237400079c837456278bcb26d" "$ROADMAP_DOC"
assert_not_contains "Roadmap removes the resolved Nurse Joy discrepancy" "Treat this as an unresolved acceptance discrepancy" "$ROADMAP_DOC"
assert_contains "Roadmap records the npm-authenticated dry-run blocker" "npm auth is currently unavailable" "$ROADMAP_DOC"
assert_contains "Roadmap quarantines the uncleared candidate" "QUARANTINED and must not be published" "$ROADMAP_DOC"
assert_contains "Roadmap records all packaged branded PNGs" "55 packaged branded PNGs" "$ROADMAP_DOC"
assert_contains "Roadmap distinguishes sprites from contact sheets" "53 theme sprites plus two contact sheets" "$ROADMAP_DOC"
assert_contains "Roadmap records exact PokeAPI HOME provenance" "byte-for-byte match PokeAPI HOME files" "$ROADMAP_DOC"
assert_contains "Roadmap preserves assets during evidence review" "Preserve all current assets while completing" "$ROADMAP_DOC"
assert_not_contains "Roadmap no longer recommends blanket asset removal" "branded-asset-free replacement candidate" "$ROADMAP_DOC"
assert_contains "Roadmap makes npm authentication secondary" "npm authentication is secondary" "$SUPERVISED_ROADMAP"
assert_contains "Roadmap preserves history unless explicitly authorized" "No published Git history rewrite is inferred" "$SUPERVISED_ROADMAP"
