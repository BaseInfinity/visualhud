...
assert_contains "Calibration text starts with step count" "[01/42]" "$CALIBRATION_TEXT"
...
assert_contains "Power Rangers calibration reaches Ultrazord" \
    ...
    "$(jq -r '[.entries[] | "\(.kind):\(.name):\(.badge):\(.color | join("-"))"] | join(",")' "$POWER_RANGERS_JSON")"
assert_contains "Power Rangers calibration includes Rita blocked state" \
    "blocked:Rita Repulsa:RITA:100-30-120" \
    "$(jq -r '[.entries[] | "\(.kind):\(.name):\(.badge):\(.color | join("-"))"] | join(",")' "$POWER_RANGERS_JSON")"
...
assert_contains "Live calibration prints first step" "[01/42]" "$LIVE_OUTPUT"
assert_contains "Live calibration prints second step" "[02/42]" "$LIVE_OUTPUT"
