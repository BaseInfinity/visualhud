...
assert_eq "PreCompact VisualHUD hook registered" "true" "$(jq -r 'any(.hooks.PreCompact[]?.hooks[]?; .command | contains("visualhud-claude.sh"))' "$SETTINGS_JSON")"
assert_eq "PostCompact VisualHUD hook registered" "true" "$(jq -r 'any(.hooks.PostCompact[]?.hooks[]?; .command | contains("visualhud-claude.sh"))' "$SETTINGS_JSON")"
assert_eq "SubagentStart VisualHUD hook registered" "true" "$(jq -r 'any(.hooks.SubagentStart[]?.hooks[]?; .command | contains("visualhud-claude.sh"))' "$SETTINGS_JSON")"
assert_eq "SubagentStop VisualHUD hook registered" "true" "$(jq -r 'any(.hooks.SubagentStop[]?.hooks[]?; .command | contains("visualhud-claude.sh"))' "$SETTINGS_JSON")"
assert_eq "PostToolUseFailure VisualHUD hook registered" "true" "$(jq -r 'any(.hooks.PostToolUseFailure[]?.hooks[]?; .command | contains("visualhud-claude.sh"))' "$SETTINGS_JSON")"
