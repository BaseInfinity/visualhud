...
assert_contains "Setup skill distinguishes Codex registration from next-hook changes" "hook or skill registration changed" "$SETUP_DOC"
assert_contains "Setup skill reserves terminal restart for changed preferences" "terminal preferences actually changed" "$SETUP_DOC"
...
assert_contains "Update skill follows the installer's exact restart phase" "reopen Codex only when" "$UPDATE_DOC"
...
assert_contains "README separates next-hook from Codex restart changes" "Runtime and theme changes apply on the next hook" "$README_DOC"
