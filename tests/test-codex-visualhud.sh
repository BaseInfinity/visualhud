...
assert_eq "PreCompact SDLC compact guard registered" "true" "$(jq -r 'any(.hooks.PreCompact[]?.hooks[]?; .command == "node .codex/hooks/compact-guard.cjs")' "$HOOKS_JSON")"
assert_eq "PostCompact SDLC compact guard registered" "true" "$(jq -r 'any(.hooks.PostCompact[]?.hooks[]?; .command == "node .codex/hooks/compact-guard.cjs")' "$HOOKS_JSON")"
assert_eq "SessionStart SDLC node hook registered" "true" "$(jq -r 'any(.hooks.SessionStart[]?.hooks[]?; .command == "node .codex/hooks/session-start.cjs")' "$HOOKS_JSON")"
echo ""
